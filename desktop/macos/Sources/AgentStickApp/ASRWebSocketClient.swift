...
final class ASRWebSocketClient: ASRClient {
    ...
    private let queue = DispatchQueue(label: "AgentStick.ASRWebSocketClient")
    ...
    private var providerWebSocketURL: String {
        switch config.asrProvider {
        case .agentStickCloud:
            return config.agentStickCloudURL.trimmingCharacters(in: .whitespacesAndNewlines)
        case .volcengine:
    ...
    private func connectWebSocket() {
        ...
        let appKey = config.volcengineAppKey.trimmingCharacters(in: .whitespacesAndNewlines)
        if config.asrProvider == .volcengine, !appKey.isEmpty {
            request.setValue(appKey, forHTTPHeaderField: "X-Api-App-Key")
            request.setValue(providerAPIKey, forHTTPHeaderField: "X-Api-Access-Key")
        ...
        if config.asrProvider == .agentStickCloud, let deviceID = config.pairedDeviceIDs.first {
