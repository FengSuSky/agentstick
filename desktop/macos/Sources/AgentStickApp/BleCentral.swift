...
struct ConnectedAgentStickDevice {
    let name: String
    let deviceID: String
}
...
final class BleCentral: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    enum FirmwareUpdateError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .noConnectedDevice:
                return L10n.noAgentStickConnected
            case .otaCharacteristicUnavailable:
                return L10n.firmwareNoBLEOTA
            case .imageTooLarge:
                return L10n.firmwareImageTooLarge
            case .transferAlreadyActive:
                return L10n.firmwareUpdateAlreadyRunning
            case .firmwareUpdateCancelled:
                return L10n.firmwareUpdateCancelled
            case .peripheralWriteFailed(let message):
                return L10n.bleWriteFailed(message)
            case .deviceError(let code):
                return L10n.deviceRejectedOTA(code)
            }
        }
    }
    ...
    private var discoveredDevices: [UUID: ConnectedAgentStickDevice] = [:]
    private var connectedDevices: [UUID: ConnectedAgentStickDevice] = [:]
    ...
    private func connectedDevice(localName: String?, peripheralName: String?) -> ConnectedAgentStickDevice? {
        let advertisedName = localName ?? peripheralName ?? ""
        guard let deviceID = Self.deviceID(from: advertisedName) else { return nil }
        return ConnectedAgentStickDevice(
            name: advertisedName.isEmpty ? "VS-\(deviceID)" : advertisedName,
            deviceID: deviceID
        )
    }

    private var currentConnectedDevices: [ConnectedAgentStickDevice] {
    ...
    private func knownDevice(for peripheral: CBPeripheral) -> ConnectedAgentStickDevice? {
