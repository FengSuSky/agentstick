...
final class FirmwareUpdateWindowController: NSWindowController {
    private let titleLabel = NSTextField(labelWithString: L10n.updatingFirmware)
    private let detailLabel = NSTextField(labelWithString: L10n.preparingUpdate)
    private let progressIndicator = NSProgressIndicator()
    private let percentLabel = NSTextField(labelWithString: "0%")
    private let speedLabel = NSTextField(labelWithString: "Speed --")
    private let timeLabel = NSTextField(labelWithString: L10n.estimatingTimeRemaining)
    private let cancelButton = NSButton(title: L10n.cancel, target: nil, action: nil)
    private let closeButton = NSButton(title: L10n.close, target: nil, action: nil)
    ...
    init(fileName: String) {
        ...
        window.title = L10n.firmwareUpdate
    ...
    func update(progress: FirmwareUpdateProgress) {
        ...
        speedLabel.stringValue = L10n.speed(Self.format(bytesPerSecond: bytesPerSecond))
        ...
        if bytesPerSecond > 1 && displayedBytes < progress.totalBytes {
            ...
            timeLabel.stringValue = L10n.finishingOnDevice
        }
    }
    ...
    func finish(result: Result<Void, Error>) {
        ...
        switch result {
        case .success:
            titleLabel.stringValue = L10n.firmwareUpdated
            detailLabel.stringValue = L10n.deviceRebooting
            progressIndicator.doubleValue = 100
            percentLabel.stringValue = "100%"
            timeLabel.stringValue = L10n.done
        case .failure(let error):
            titleLabel.stringValue = L10n.updateFailed
            detailLabel.stringValue = error.localizedDescription
            timeLabel.stringValue = L10n.keptCurrentFirmware
        }
    }
    ...
    @objc private func cancelUpdate() {
        cancelButton.isEnabled = false
        titleLabel.stringValue = L10n.cancellingFirmwareUpdate
        detailLabel.stringValue = L10n.stoppingTransfer
        timeLabel.stringValue = L10n.cancelling
        onCancel?()
    }
