...
final class PairDeviceWindowController: NSWindowController, CBCentralManagerDelegate, NSTableViewDataSource, NSTableViewDelegate {
    ...
    private let statusLabel = NSTextField(labelWithString: L10n.scanning)
    ...
    init(existingDeviceIDs: [String], onPair: @escaping (String) -> Void) {
        ...
        window.title = L10n.pairAgentStickTitle
    ...
    private func buildContent() {
        ...
        let pairButton = NSButton(title: L10n.pair, target: self, action: #selector(pairSelectedDevice))
    ...
    func centralManagerDidUpdateState(_ central: CBCentralManager) {
        guard central.state == .poweredOn else {
            statusLabel.stringValue = L10n.bluetoothUnavailable
            return
        }
        statusLabel.stringValue = L10n.scanning
    ...
    @objc private func pairSelectedDevice() {
        ...
        guard row >= 0, row < devices.count else {
            statusLabel.stringValue = L10n.selectADevice
            return
        }
        central?.stopScan()
