...
final class StatusController {
    ...
    private var connectedDevices: [ConnectedAgentStickDevice] = []
    ...
    func setConnectedDevices(_ devices: [ConnectedAgentStickDevice]) {
    ...
    private func addInputItems() {
        ...
        let afterPasteItem = makeMenuItem(
            title: L10n.pressReturnAfterPaste,
            symbolName: "return",
    ...
    private func addDeviceTextItems(to submenu: NSMenu, deviceID: String) {
        ...
        for language in Self.translationTargets {
            let item = NSMenuItem(
                title: L10n.translateTo(language.name),
                action: #selector(selectDeviceTextMode),
                keyEquivalent: ""
            )
            item.target = self
    ...
    private func addFirmwareItems(to submenu: NSMenu, deviceID: String, isConnected: Bool) {
        ...
        let currentTitle = info?.currentVersion.map { L10n.firmware($0) } ?? L10n.firmwareUnknown
        ...
        if info?.isChecking == true {
            let checkingItem = NSMenuItem(title: L10n.checkingForUpdates, action: nil, keyEquivalent: "")
        ...
        if let errorMessage = info?.errorMessage {
            let errorItem = NSMenuItem(title: L10n.updateCheckFailed, action: nil, keyEquivalent: "")
        ...
        if info?.updateAvailable == true, let latestVersion = info?.latestVersion {
            let updateItem = makeMenuItem(
                title: L10n.updateTo(latestVersion),
            ...
            let upToDateItem = NSMenuItem(title: L10n.firmwareUpToDate, action: nil, keyEquivalent: "")
            upToDateItem.isEnabled = false
            upToDateItem.image = Self.symbolImage(named: "checkmark.circle", accessibilityDescription: L10n.firmwareUpToDate)
    ...
    func showFinal(_ text: String, deviceID: String? = nil, onHidden: (() -> Void)? = nil) {
        setStatus(text.isEmpty ? L10n.noSpeech : "Ready")
        let overlay = overlay(for: deviceID)
        markOverlayVisible(for: deviceID)
        applyOverlayStyle(for: deviceID, overlay: overlay)
    ...
    private func updateStatusButton(_ status: AppStatus) {
        ...
        button.toolTip = "AgentStick: \(status.accessibilityDescription)"
        button.setAccessibilityLabel("AgentStick: \(status.accessibilityDescription)")
    }
