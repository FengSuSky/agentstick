...
/* ── M5PM1 PMIC registers (kept for code that still references them) ── */
...
{
    if (!s_pmic_dev) return ESP_ERR_INVALID_STATE;
...
static esp_err_t pca9557_update_reg(uint8_t reg, uint8_t clear_mask, uint8_t set_mask)
{
    uint8_t value = 0;
    esp_err_t err = pca9557_read_reg(reg, &value);
    if (err != ESP_OK) {
        return err;
    }
...
static esp_err_t init_i2c(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "I2C port 0 sda=%d scl=%d init failed: %s",
                 STICK_S3_PIN_I2C_SDA, STICK_S3_PIN_I2C_SCL, esp_err_to_name(err));
        return err;
    }
...
static void init_lcd_io_expander(void)
{
    ...
    err = pca9557_read_reg(PCA9557_REG_OUTPUT, &output);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "PCA9557 read output failed: %s", esp_err_to_name(err));
        return;
    }

    err = pca9557_read_reg(PCA9557_REG_CONFIG, &config);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "PCA9557 read config failed: %s", esp_err_to_name(err));
        return;
    }

    ESP_LOGI(TAG, "PCA9557 output=0x%02x config=0x%02x (expected 0x%02x, 0x%02x)",
             output, config, PCA9557_DEFAULT_OUTPUT, PCA9557_DEFAULT_CONFIG);

    if (output != PCA9557_DEFAULT_OUTPUT) {
        ESP_ERROR_CHECK_WITHOUT_ABORT(pca9557_write_reg(PCA9557_REG_OUTPUT,
                                                        PCA9557_DEFAULT_OUTPUT));
    }
    if (config != PCA9557_DEFAULT_CONFIG) {
...
esp_err_t stick_s3_board_init(void)
{
    ...
    if (err != ESP_OK) {
        ...
        init_pmic();            /* no-op on Lichuang board */
...
esp_err_t stick_s3_board_lcd_select(bool selected)
{
    ...
    esp_err_t err = pca9557_update_reg(PCA9557_REG_OUTPUT,
                                       selected ? PCA9557_LCD_CS : 0,
                                       selected ? 0 : PCA9557_LCD_CS);
    if (err == ESP_OK) {
        uint8_t output = 0;
        if (pca9557_read_reg(PCA9557_REG_OUTPUT, &output) == ESP_OK) {
            ESP_LOGI(TAG, "LCD_CS %s output=0x%02x", selected ? "low" : "high", output);
        }
    }
    return err;
#else
