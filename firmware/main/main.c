...
{
    if (s_display_dimmed) {
        esp_err_t err = ui_status_set_brightness_stop_fade(DISPLAY_ACTIVE_BRIGHTNESS);
        if (err != ESP_OK) {
...
static void display_dim_timer_cb(void *arg)
{
    ...
    if (!s_display_dimmed && !s_recording && !s_ota_updating) {
        esp_err_t err = ui_status_set_brightness_stop_fade(DISPLAY_DIM_BRIGHTNESS);
        if (err == ESP_OK) {
