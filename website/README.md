...
Vue + Vite source for the AgentStick homepage and Sparkle/WinSparkle appcast. The site uses
...
1. Generate Sparkle keys once and keep the private key out of git:
   ...
   .build/artifacts/sparkle/Sparkle/bin/generate_keys --account agentstick
   ```
...
5. Update `website/public/appcast.xml`:
   ...
   - `sparkle:edSignature`: content from `build/AgentStick-<version>.signature`
   - `length`: byte size from `wc -c build/AgentStick-<version>.zip`
...
- `scripts/build-msi.bat` runs on the local Windows signing machine with the USB signing key inserted. Upload the generated `AgentStick_<version>.msi` to the matching GitHub Release.
